 ...
 * through {@see \ArrayPress\RegisterReports\Utils\Runtime} instead.
 *
...
final class NoSharedRuntimeKeysTest extends TestCase {
	...
	/**
	 * The upload subdirectory holding generated exports.
	 */
	public function test_export_directory_is_not_a_literal(): void {
		$found = [];

		foreach ( $this->sources() as $path => $code ) {
			if ( str_contains( $code, "'reports-exports'" ) ) {
				$found[] = $path;
			}
		}
